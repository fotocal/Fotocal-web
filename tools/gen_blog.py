...
{strip}      </div>
    ...
    <div class="container-narrow">
{care_top}      <div class="post-body reveal" data-lang-block="en">
{body_en}
      </div>
...
{care}      <p class="post-disclaimer reveal" data-i18n="blog.disclaimer">This article is general information, not medical advice.</p>
...
CARE = """      <!--# Set "caution": true in the post's JSON. The wording is the one the
           weight-loss feature page carries (sp.weight-loss.care.*), and it is
           reserved for posts about deficits, rates of loss, fasting, cravings
           or binge eating — not applied by reflex. -->
      <aside class="ai-callout post-care reveal">
        <div class="ai-callout-ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 3l8 3.5v5c0 5-3.4 8.9-8 10.5-4.6-1.6-8-5.5-8-10.5v-5z"/><path d="M12 8v4M12 16h.01"/></svg></div>
        <div>
          <h4 data-i18n="blog.care.h">This is not medical advice</h4>
          <p data-i18n="blog.care.p">If you are pregnant, living with diabetes or another condition, taking medication, under eighteen, or have any history of an eating disorder, please speak to a doctor or a registered dietitian before changing how you eat or using a calorie tracker. Their advice comes first, and nothing here overrides it.</p>
        </div>
      </aside>
"""

REDIRECT = """<!DOCTYPE html>
<!-- GENERATED by tools/gen_blog.py from tools/blog/redirects.json: the post at this
     URL was retired and its one useful idea lives at the target. A live, indexed
     URL must never 404, so this stub redirects (meta refresh 0 = a permanent
     redirect to search engines), declares the target canonical and is noindex,
     which tools/build_site.py reads to keep it out of the sitemap. -->
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>{title}</title>
  <meta name="description" content="This article moved to {title}.">
  <meta name="robots" content="noindex, follow">
  <meta http-equiv="refresh" content="0; url=../{target}/">
  <link rel="canonical" href="https://getfotocal.com/blog/{target}/">
  <link rel="stylesheet" href="../../assets/css/fonts.css">
  <link rel="stylesheet" href="../../assets/css/site.css">
</head>
<body>
  <main id="main" class="container-narrow" style="padding:3rem 0">
    <h1 data-lang-block="en">{title_en}</h1>
    <h1 data-lang-block="es">{title_es}</h1>
    <p class="art-lead" data-lang-block="en">This article was retired and its content now lives in the article below. You are being taken there automatically.</p>
    <p class="art-lead" data-lang-block="es">Este artículo se retiró y su contenido vive ahora en el artículo de abajo. Te llevamos allí automáticamente.</p>
    <p data-lang-block="en"><a href="../{target}/">Continue to {title_en}</a></p>
    <p data-lang-block="es"><a href="../{target}/">Ir a {title_es}</a></p>
  </main>
</body>
</html>
"""
...
if __name__ == "__main__":
    ...
    for old_slug, target in redirects.items():
        assert target in by_slug, ("redirect target missing", old_slug, target)
        assert old_slug not in by_slug, ("redirect shadows a live post", old_slug)
        t = by_slug[target]
        d = os.path.join(SRC, old_slug)
        os.makedirs(d, exist_ok=True)
        open(os.path.join(d, "index.html"), "w", encoding="utf-8").write(REDIRECT.format(
            target=target, title=esc(strip_tags(t["en"]["title"])), title_en=esc(strip_tags(t["en"]["title"])), title_es=esc(strip_tags(t["es"]["title"]))))
    print("redirect stubs:", len(redirects))
    inb = inbound()
